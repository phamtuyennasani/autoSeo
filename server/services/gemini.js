...
async function generateTitles(keyword, searchContext, count = 10, userConfig = {}) {
  const keysStr = userConfig.apiKey || process.env.GEMINI_API_KEY;
  if (!keysStr) throw new Error('GEMINI_API_KEY chưa được cấu hình. Vào Cài đặt → Cấu hình API để nhập key.');
