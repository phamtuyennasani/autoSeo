const { stripHtmlTags } = require('../utils/func');
 ...
 */
function buildArticlePrompt(keyword, title, company, optionsPromt = '', customLinks = '', imageUrls = '') {
  const optionsPromtbyUser = optionsPromt ? `\n\n**YÊU CẦU BỔ SUNG từ người dùng:** ${optionsPromt}\n` : '';

  // Chèn custom links vào prompt nếu có
  const customLinksBlock = customLinks
    ? `\n\n**DANH SÁCH URL LIÊN KẾT — chèn dưới dạng markdown link [anchor text](url) tự nhiên vào bài viết:**\n${customLinks}\n`
    : '';
...
- ** Không được chèn liên kết vào H2, H3. Chỉ chèn link tự nhiên trong đoạn văn, và chỉ chèn tối đa 1 link mỗi đoạn.**
...
## REMINDER: Return ONLY valid JSON. No markdown. No explanation.
