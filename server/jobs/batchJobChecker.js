...
const { LOG,CHECK_INTERVAL_MS,SCHEDULE_TICK_MS } = require('../utils/func');
...
async function checkScheduledRun() {
  try {
    ...
    LOG(`[BatchJobChecker] Kích hoạt theo lịch (${scheduleTime})`);
    await checkPendingJobs();
  } catch (err) {
    LOG(`[BatchJobChecker] Lỗi checkScheduledRun: ${err.message}`);
  }
}
...
async function importJobResults(job, results) {
  ...
  for (const result of results) {
    try {
      ...
      if (outcome.saved) {
        succeeded++;
        LOG(`[BatchJobChecker]  ✅ Đã lưu: "${result.title}"`);
      } else if (outcome.skipped) {
        succeeded++;
        LOG(`[BatchJobChecker]  ⏭ Bỏ qua (đã có): "${result.title}"`);
      } else {
        failed++;
        LOG(`[BatchJobChecker]  ❌ Lỗi: "${result.title}" — ${outcome.message}`);
      }
    } catch (e) {
      failed++;
      LOG(`[BatchJobChecker]  ❌ Exception lưu "${result.title}": ${e.message}`);
    }
  }
...
async function submitScheduledJobs() {
  ...
  LOG(`[BatchJobChecker] Có ${jobs.length} job hẹn giờ đến lượt gửi...`);

  for (const job of jobs) {
    LOG(`[BatchJobChecker] Submitting job ${job.id} — keyword: "${job.keyword}"`);
    try {
      ...
      LOG(`[BatchJobChecker]  ✅ Đã gửi! gemini_job_name: ${geminiJobName}`);
    } catch (err) {
      LOG(`[BatchJobChecker]  ❌ Lỗi submit job ${job.id}:`, err.message);
    }
  }
}
...
async function checkPendingJobs() {
  ...
  if (pendingJobs.length === 0) {
    LOG(`[BatchJobChecker] Không có job nào đang chờ.`);
    return;
  }

  LOG(`[BatchJobChecker] Bắt đầu check ${pendingJobs.length} job(s)...`);

  for (const job of pendingJobs) {
    LOG(`[BatchJobChecker] Checking job ${job.id} — keyword: "${job.keyword}"`);
    const titles = JSON.parse(job.titles || '[]');
    ...
    try {
      ...
      if (!result.done) {
        LOG(`[BatchJobChecker]  Job chưa xong. State: ${result.state}`);
        continue;
      }

      if (result.failed) {
        LOG(`[BatchJobChecker]  Job thất bại. State: ${result.state}`);
        await db.execute({
          sql: `UPDATE batch_jobs SET status = 'failed', completedAt = ? WHERE id = ?`,
          args: [new Date().toISOString(), job.id],
        });
      ...
      LOG(`[BatchJobChecker]  Job SUCCEEDED! Đang import ${result.results.length} bài...`);
      ...
      LOG(`[BatchJobChecker]  Xong! Thành công: ${succeeded}, Lỗi: ${failed}`);
    } catch (err) {
      LOG(`[BatchJobChecker]  Lỗi khi check job ${job.id}:`, err.message);
    }
  }

  LOG(`[BatchJobChecker] Hoàn thành lượt check.`);
}

// ─── Khởi động scheduler ─────────────────────────────────────────────────────
function startBatchJobChecker() {
  LOG(`[BatchJobChecker] Scheduler khởi động. Check định kỳ mỗi ${CHECK_INTERVAL_MS / 60000} phút.`);

  // Check lần đầu sau 10 giây khởi động
  setTimeout(() => {
    checkPendingJobs().catch(err => LOG(`[BatchJobChecker] Lỗi lần check đầu:`, err.message));
  }, 10_000);

  // Check định kỳ mỗi 60 phút
  setInterval(() => {
    checkPendingJobs().catch(err => LOG(`[BatchJobChecker] Lỗi check định kỳ:`, err.message));
  }, CHECK_INTERVAL_MS);

  // Tick mỗi phút: submit job đến giờ hẹn + kiểm tra lịch check chung
  setInterval(() => {
    submitScheduledJobs().catch(err => LOG(`[BatchJobChecker] Lỗi submitScheduledJobs:`, err.message));
    checkScheduledRun().catch(err => LOG(`[BatchJobChecker] Lỗi schedule tick:`, err.message));
  }, SCHEDULE_TICK_MS);
}
